...
df = df.drop(columns=['eid', '20016-2.0', '21003-2.0'], axis=1)
...
def predict_collect_save(data_pred, data_collect, y_true, test_index,
    ...
    scores['target'] = 'Age at assessment'
    scores['variable'] = 'EL, EDU, LS, MH'
...
# save outputs
savedir = join('outputs', 'combined_age_at_assessment_prediction_from_all_variables')
...
def collect_ids(model_testing, data, outputs):
    ...
    to_save['target'] = pd.Series(['Age at assessment'] * len(to_save),
                                  index=to_save.index)
    to_save['variable'] = pd.Series(['EL, EDU, LS, MH'] * len(to_save),
                                    index=to_save.index)
