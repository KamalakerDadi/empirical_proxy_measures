...
X_iterate = zip([brain_dmri_fa, brain_dmri_icvf, brain_dmri_isovf, brain_dmri_l1,
                 ...
                 brain_dmri_od, brain_smri_plus, education, fluid_intelligence],
                 ['fa', 'icvf', 'isovf', 'l1', 'l2', 'l3', 'md', 'mo', 'od',
                  'smri', 'Education', 'Fluid \n intelligence'])
...
df = df.drop(columns=['eid', '20016-2.0', '21003-2.0'],
             axis=1)
df_test = df_test.drop(columns=['eid', '20016-2.0', '21003-2.0'],
                       axis=1)
...
def predict_collect_save(data_pred, data_collect, y_true, test_index,
    ...
    scores['target'] = 'Age'
    scores['Data type'] = 'Imaging & Non-imaging'
...
# save outputs
savedir = join('outputs', 'combined_age_at_assessment_prediction_from_education')
...
def collect_ids(model_testing, data, outputs):
    ...
    to_save['variable'] = pd.Series(['Education'] * len(to_save),
                                    index=to_save.index)
    to_save['Data type'] = pd.Series(['Imaging & Non-imaging'] * len(to_save),
                                     index=to_save.index)
