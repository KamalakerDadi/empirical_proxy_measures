...
def load_combine_data(X_split, merged_data):
    ...
    X_split = X_split.drop(columns=['eid', '20016-2.0', '21003-2.0'],
                           axis=1)
...
def predict_collect_save(data_pred, data_collect, y_true, test_index,
    ...
    predictions = pd.DataFrame(pred_, columns=['predicted'], index=y_true_.index)
...
# save outputs
savedir = join('outputs', 'imaging_age_at_assessment_prediction_from_brain_volumes')
