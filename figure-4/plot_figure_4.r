...
data <- read.csv("./inputs/classification.csv")
data2 <- read.csv("./inputs/classification_non_imaging_fresh.csv")
...
data_brain_agg <- read.csv(
  "./brain_inference_class.csv")[,c("target", "t_obs")]
names(data_brain_agg)[[2]] <- "roc"
data_brain_agg$Type <- "Imaging"
setDT(data_brain_agg)
...
dat1$target[dat1$target == "Fluid intelligence"] <- "Fluid \nIntelligence"
dat3$target[dat3$target == "Fluid intelligence"] <- "Fluid \nIntelligence"
...
fname <- 'figure_4'
my_ggsave(fname = fname, plot = fig, height = 3.3, width = 6)
