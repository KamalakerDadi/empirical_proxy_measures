...
base_path <- file.path(
    "inputs",
    'post_predictive_analysis_class_%s.csv')
...
DT_inf_brain <- DT_brain[evaluation == "Generalization",
  {
    ...
    perm_auc <- par_sapply(perms, function(perm){
      set.seed(perm)
...
#'Small concern: Numbers in stats look slightly higher
#'Is it because we average before computing AUC. Let's compute it fold-wise

DT_brain2 <- fread(sprintf(base_path, "full_mri"))
DT_brain_auc <- DT_brain2[
  evaluation == "Generalization",
  {
     dat <- na.omit(data.frame(true = true,
                               predicted = predicted))
     auc =  roc_auc_score(true = dat$true, predicted = dat$predicted)
    .(auc = auc)
  },
  by = .(target, fold)]

knitr::kable(DT_brain_auc[,.(auc = mean(auc)), .(target)])
