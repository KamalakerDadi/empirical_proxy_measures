...
from ukbb_variables import (education, earlylife, lifestyle,
                            mental_health, primary_demographics,
...
X_iterate = zip([brain_dmri_fa, brain_dmri_icvf, brain_dmri_isovf, brain_dmri_l1,
                 ...
                 education, earlylife, lifestyle, mental_health, primary_demographics],
...
def split_data(X_split, y_split, group):
    ...
    low_group = X_split[X_split['20016-2.0'] < quantiles[0.333]]
    low_group = low_group.drop(columns=['20016-2.0'], axis=1)
    low_group['class_type'] = pd.Series([0] * len(low_group),
                                        index=low_group.index)

    middle_group = X_split[(X_split['20016-2.0'] >= 6.0) &
                           (X_split['20016-2.0'] <= 8.0)]
    middle_group = middle_group.drop(columns=['20016-2.0'], axis=1)
    middle_group['class_type'] = pd.Series([1] * len(middle_group),
                                           index=middle_group.index)

    high_group = X_split[X_split['20016-2.0'] > quantiles[0.666]]
    high_group = high_group.drop(columns=['20016-2.0'], axis=1)
...
X_iterate = zip([education, earlylife, lifestyle, mental_health, primary_demographics],
                ['Education', 'Earlylife', 'Lifestyle', 'Mental health', 'Demo'])
new_columns = []
for i in X_iterate:
    new_columns.extend(i[0].keys())
new_columns.extend(['eid', '20016-2.0'])
...
def predict_collect_save(data_pred, data_collect, y_true, test_index,
    ...
    scores['target'] = 'Fluid intelligence'
...
# save outputs
savedir = join('outputs', 'classification_non_imaging_fluid_intelligence_prediction')
...
def collect_ids(model_testing, data, outputs):
    # locate ids
    subjects = data.iloc[outputs['test_indices']][['20016-2.0', 'eid']]
    to_save = pd.concat([outputs, subjects], axis=1)
    to_save['target'] = pd.Series(['Fluid intelligence'] * len(to_save),
                                  index=to_save.index)
