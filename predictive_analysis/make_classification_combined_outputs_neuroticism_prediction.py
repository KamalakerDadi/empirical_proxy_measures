...
def split_data(X_split, y_split, group):
    ...
    low_group = X_split[X_split['20127-0.0'] < quantiles[0.333]]
    low_group = low_group.drop(columns=['20127-0.0'], axis=1)
    low_group['class_type'] = pd.Series([0] * len(low_group),
                                        index=low_group.index)

    middle_group = X_split[(X_split['20127-0.0'] >= 2.0) &
                           (X_split['20127-0.0'] <= 5.0)]
    middle_group = middle_group.drop(columns=['20127-0.0'], axis=1)
    middle_group['class_type'] = pd.Series([1] * len(middle_group),
                                           index=middle_group.index)

    high_group = X_split[X_split['20127-0.0'] > quantiles[0.666]]
    high_group = high_group.drop(columns=['20127-0.0'], axis=1)
...
df = df.drop(columns=['eid', '20016-2.0', '20127-0.0'], axis=1)
df_test = df_test.drop(columns=['eid', '20016-2.0', '20127-0.0'], axis=1)
...
# save outputs
savedir = join('outputs', 'classification_combined_neuroticism_prediction')
...
def collect_ids(model_testing, data, outputs):
    # locate ids
    subjects = data.iloc[outputs['test_indices']][['20127-0.0', 'eid']]
    to_save = pd.concat([outputs, subjects], axis=1)
    to_save['target'] = pd.Series(['Neuroticism'] * len(to_save),
                                  index=to_save.index)
