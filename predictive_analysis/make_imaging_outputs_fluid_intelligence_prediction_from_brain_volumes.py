...
def load_combine_data(X_split, merged_data):
    ...
    X_split = X_split.drop(columns=['eid', '20016-2.0'], axis=1)
...
y_test = pd.DataFrame(y_test, index=df_test.index)
...
def predict_collect_save(data_pred, data_collect, y_true, test_index,
    ...
    scores['modality'] = 'brain volumes (sMRI)'
...
# save outputs
savedir = join('outputs', 'imaging_fluid_intelligence_prediction_from_brain_volumes')
