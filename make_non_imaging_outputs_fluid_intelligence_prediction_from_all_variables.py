...
def predict_collect_save(data_pred, data_collect, y_true, test_index,
    ...
    scores['variable'] = 'EL, EDU, LS, MH, AS'
...
# save outputs
savedir = join('outputs', 'non_imaging_fluid_intelligence_prediction_from_all_variables')
...
def collect_ids(model_testing, data, outputs):
    ...
    to_save['target'] = pd.Series(['Fluid intelligence'] * len(to_save),
                                  index=to_save.index)
    to_save['variable'] = pd.Series(['EL, EDU, LS, MH, AS'] * len(to_save),
                                    index=to_save.index)
