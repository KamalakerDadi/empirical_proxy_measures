...
from ukbb_variables import (brain_dmri_fa, brain_dmri_icvf,
                            ...
                            mental_health, fluid_intelligence)
...
X_iterate = zip([brain_dmri_fa, brain_dmri_icvf, brain_dmri_isovf, brain_dmri_l1,
                 ...
                 neuroticism, mental_health],
                 ['fa', 'icvf', 'isovf', 'l1', 'l2', 'l3', 'md', 'mo', 'od',
                  'smri', 'Fluid \n intelligence', 'Neuroticism', 'Mental health'])
...
def predict_collect_save(data_pred, data_collect, y_true, test_index,
    ...
    scores['variable'] = 'Mental health (MH)'
...
# save outputs
savedir = join('outputs', 'combined_neuroticism_prediction_from_mental_health')
...
def collect_ids(model_testing, data, outputs):
    ...
    to_save['target'] = pd.Series(['Neuroticism'] * len(to_save),
                                  index=to_save.index)
    to_save['variable'] = pd.Series(['Mental health'] * len(to_save),
                                    index=to_save.index)
