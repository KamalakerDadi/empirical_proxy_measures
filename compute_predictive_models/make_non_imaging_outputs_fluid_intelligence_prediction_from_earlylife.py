...
X_iterate = zip([earlylife], ['Earlylife'])
new_columns = []
for i in X_iterate:
...
# save outputs
savedir = join('outputs', 'non_imaging_fluid_intelligence_prediction_from_earlylife')
...
def collect_ids(model_testing, data, outputs):
    ...
    to_save['target'] = pd.Series(['Fluid intelligence'] * len(to_save),
                                  index=to_save.index)
    to_save['variable'] = pd.Series(['Earlylife'] * len(to_save),
                                    index=to_save.index)
