...
def predict_collect_save(data_pred, data_collect, y_true, test_index,
    ...
    scores['Type'] = 'Imaging & Non-imaging'
...
# save outputs
savedir = join('outputs', 'classification_combined_fluid_intelligence_prediction')
...
def collect_ids(model_testing, data, outputs):
    ...
    to_save['group'] = pd.Series(['low vs high'] * len(to_save),
                                 index=to_save.index)
    to_save['Type'] = pd.Series(['Imaging & Non-imaging'] * len(to_save),
                                index=to_save.index)
