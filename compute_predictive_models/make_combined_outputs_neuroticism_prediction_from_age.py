...
X_iterate = zip([brain_dmri_fa, brain_dmri_icvf, brain_dmri_isovf, brain_dmri_l1,
                 ...
                 neuroticism, primary_demographics],
                 ['fa', 'icvf', 'isovf', 'l1', 'l2', 'l3', 'md', 'mo', 'od',
                  'smri', 'Fluid \n intelligence', 'Neuroticism', 'Demographics'])
...
# save outputs
savedir = join('outputs', 'combined_neuroticism_prediction_from_age')
...
def collect_ids(model_testing, data, outputs):
    ...
    to_save['target'] = pd.Series(['Neuroticism'] * len(to_save),
                                  index=to_save.index)
    to_save['variable'] = pd.Series(['Age, Sex'] * len(to_save),
                                    index=to_save.index)
