...
X_iterate = zip([brain_dmri_fa, brain_dmri_icvf, brain_dmri_isovf, brain_dmri_l1,
                 ...
                 brain_dmri_od, brain_smri_plus, fluid_intelligence, education],
...
def load_combine_data(X_split, merged_data, dmri):
    ...
    y_split = pd.DataFrame(X_split, columns=['20016-2.0'])
...
# save outputs
savedir = join('outputs', 'non_imaging_fluid_intelligence_prediction_from_education')
...
def collect_ids(model_testing, data, outputs):
    ...
    to_save['target'] = pd.Series(['Fluid intelligence'] * len(to_save),
                                  index=to_save.index)
    to_save['variable'] = pd.Series(['Education'] * len(to_save),
                                    index=to_save.index)
