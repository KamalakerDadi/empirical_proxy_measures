...
#' get significance
DT_corr3[, significant := ifelse(DT_corr3$p.val < 0.05, "yes", "no")]
writeLines(
  knitr::kable(DT_corr3, format = "latex"),
  "posthoc_corr_results.tex")


#' get parametric bootstrap for conditional proxy model.
DT_corr3b <- DT_pred_wide3[,
  {
    train <- .SD[evaluation == "Validation"]
    test <- .SD[evaluation == "Generalization"]
    deconf <- fit_train_deconfound(
...
DT_corr3b_small <- DT_corr3b[,
  .(coef = sample(coef, 200, replace = F)),  by = .(target, family, name)]

# get parametric bootstrap for conditional target model
DT_corr3b_target <- DT_pred_wide3[,
  {
    train <- .SD[evaluation == "Validation"]
    test <- .SD[evaluation == "Generalization"]
    deconf <- fit_train_deconfound(
...
(
  fig_corr_best_target <- plot_corr(
    data = DT_corr3b_target,
    data_mean = DT_corr3_target,
    title = 'B (target measure)')
)

my_ggsave("fig_posthoc_best_multiplereg",
          plot = fig_corr_best,
          width = 12, height = 5)

my_ggsave("fig_posthoc_best_multiplereg_target",
          plot = fig_corr_best_target,
          width = 12, height = 5)

big_fig <- (fig_corr_best / fig_corr_best_target) + 
  plot_annotation(
    title = 'Specific associations of target and proxy measures with health-related habits')

my_ggsave("fig_posthoc_combined",
          plot = big_fig,
          width = 12, height = 9)
